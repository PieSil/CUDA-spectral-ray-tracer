...
public:
	...
	__host__
	renderer(frame_buffer* fb, bvh** _bvh, uint _samples_per_pixel, camera* cam, uint _bounce_limit) :
        h_fb_data_ptr(fb->data), dev_bvh(_bvh), samples_per_pixel(_samples_per_pixel), bounce_limit(_bounce_limit), background(cam->getBackground()) {
		assign_cam_data(cam);
	...
	__host__
    void init_device_params(dim3 _threads, dim3 _blocks, uint _chunk_width, uint _chunk_height);

	__host__
	void call_render_kernel();
